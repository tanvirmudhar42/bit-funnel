...
fn main() -> anyhow::Result<()> {
    ...
    index.index_document("doc4.txt".into(), "Programming in Rust is fun.".to_string())?;
    ...
    let test_cases = vec![
        (
            "rust programming",
            vec!["doc1.txt", "doc3.txt"],
            "Should match docs with 'rust' before 'programming'",
        ),
        (
            "programming rust",
            vec!["doc4.txt"],
            "Should match doc4 (programming before rust)",
        ),
        (
            "python programming",
            vec!["doc2.txt"],
            "Should match doc with both words in order",
        ),
        (
            "rust language",
            vec!["doc1.txt", "doc3.txt"],
            "Should match docs with 'rust' before 'language'",
        ),
        (
            "programming language",
            vec!["doc1.txt", "doc3.txt"],
            "Should match docs with both words in order",
        ),
        (
            "language rust",
            vec![],
            "Should NOT match (wrong order - language comes after rust)",
        ),
    ];
    ...
    for (query, expected_files, description) in test_cases {
        ...
        if result_files.is_empty() && expected_files.is_empty() {
            ...
            && expected_files
                .iter()
                .all(|f| result_files.contains(&f.to_string()))
        {
